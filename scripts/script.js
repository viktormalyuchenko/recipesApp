...
function displayRecipeList(recipeArray = recipes) {
  ...
  recipeArray.forEach(recipe => {
    ...
    recipeItem.innerHTML = `
            ${recipe.image ? `<img src="${recipe.image}" alt="${recipe.title}" loading="lazy" data-id="${recipe.id}" class="recipe-image">` : ''}  
            <div class="recipe-item-content">
                <a href="#" class="recipe-title-link" data-id="${recipe.id}"><h2>${recipe.title}</h2></a>
    ...
    // Добавляем обработчики для картинки и заголовка
    const image = recipeItem.querySelector(".recipe-image");
    const titleLink = recipeItem.querySelector(".recipe-title-link");
...
async function loadRecipes() {
  ...
  try {
    ...
    recipeListDiv.innerHTML =
      "<p>Не удалось загрузить рецепты.  Попробуйте обновить страницу или зайти позже.</p>";
  }
}
